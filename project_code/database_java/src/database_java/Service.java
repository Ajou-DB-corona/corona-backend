...
public class Service {
	...
	public Service() {
		...
		try {
			conn = DriverManager.getConnection("jdbc:postgresql://127.0.0.1:5432/postgres","postgres","1234");
		...
		try {
			...
			while((line = br.readLine())!=null) {
				...
				String[] str = line.split(",");
				if(firstRead) {
					q="create table City("+str[0]+" int,"+str[1]+" varchar(10),"+str[2] + " float,"+str[3]+" float,"+"primary key("+str[0]+"));";
					st.execute(q);
				}
				else {
					st.execute("insert into City values("+str[0]+",'"+str[1]+"',"+str[2]+","+str[3]+");");
				}
				firstRead = false;
			}
		}
		...
		try {	
			...
			while((line = br.readLine())!=null) {
				...
				String[] str = line.split(",");
				if(firstRead) {
					st.execute("create table Place("+str[0]+" int,"+str[1]+" varchar(30),"+str[2] + " int,"+str[3]+" varchar(30),"+str[4]+" varchar(60),"+str[5]+" int,"+"primary key("+str[2]+"));");
				}
				else{
					q="insert into Place values("+str[0]+",'"+str[1]+"',"+str[2]+",'"+str[3]+"','"+str[4]+"',"+str[5]+");";
					st.execute(q);
		...
		try {	
			...
			while((line = br.readLine())!=null) {
				...
				if(firstRead) {
					st.execute("create table PlaceLocation("+str[0]+" int,"+str[1]+" float,"+str[2] + " float,"+"primary key("+str[0]+"));");
				}
				else {
	...
	public void readCategoryPlace(int categoryID,boolean orderbyStar){
		...
		try {
			...
			if(orderbyStar) {
			ret = st.executeQuery("select category,placename,number,address,count,star from place natural join category natural join (select customerid,placeid,count(star), round(avg(star)::numeric, 1) as star\r\n"
					+ "from star\r\n"
					...
					+ "where placeid not in(select placeid from star) and categoryid="+categoryID+" order by star desc;");
			}
			else{
				ret = st.executeQuery("select category,placename,number,address,count,star from place natural join category natural join (select customerid,placeid,count(star), round(avg(star)::numeric, 1) as star\r\n"
						+ "from star\r\n"
	...
	public void readCityPlace(String city,boolean orderbyStar){
		...
		try {
			...
			if(orderbyStar) {
			ret = st.executeQuery("select category,placename,number,address,count,star from place natural join category natural join city natural join (select customerid,placeid,count(star), round(avg(star)::numeric, 1) as star\r\n"
					+ "from star\r\n"
					+ "group by rollup(placeid,customerid))as S\r\n"
					...
					+ "where placeid not in(select placeid from star) and cityid="+cityID+" order by star desc;");
			}
			else{
				ret = st.executeQuery("select category,placename,number,address,count,star from place natural join category natural join city natural join (select customerid,placeid,count(star), round(avg(star)::numeric, 1) as star\r\n"
						+ "from star\r\n"
	...
	public void readCategoryCityPlace(String city, int categoryID, boolean orderbyStar){
		...
		try {
			...
			ret = st.executeQuery("select category,placename,number,address,count,star from place natural join category natural join city natural join (select customerid,placeid,count(star), round(avg(star)::numeric, 1) as star\r\n"
					...
					+ "select category,placename,number,address,0,0\r\n"
					+ "from place natural join category natural join city\r\n"
					+ "where placeid not in(select placeid from star) and cityid="+cityID+" and categoryid="+categoryID+" order by star desc;");
			}
			else{
				ret = st.executeQuery("select category,placename,number,address,count,star from place natural join category natural join city natural join (select customerid,placeid,count(star), round(avg(star)::numeric, 1) as star\r\n"
						+ "from star\r\n"
						...
						+ "select category,placename,number,address,0,0\r\n"
						+ "from place natural join category natural join city\r\n"
						+ "where placeid not in(select placeid from star) and cityid="+cityID+" and categoryid="+categoryID+";");
			}
	...
	public void readCoursePlace(String srcCity, String dstCity, boolean checkCategory, int categoryID) {
		...
		try {
			...
			srcCityID = checkCity(srcCity);
			if(srcCityID==-1) {
				throw new InputMismatchException("없는 도시.") ;
			}
				
			
			dstCityID = checkCity(dstCity);
			if(dstCityID==-1) {
				throw new InputMismatchException("없는 도시.") ;
			}
			...
			if(checkCategory) {
				...
				ret = st.executeQuery("select category, placename, number, address,latitude, longitude,count, star\r\n"
						+ "from(\r\n"
						...
						+ "    )/1000 < (ST_DistanceSphere(ST_MakePoint("+longitude[0]+","+latitude[0]+",4326), ST_MakePoint("+longitude[1]+","+latitude[1]+",4326))/1000)"+"\r\n"
						+ "union\r\n"
						...
						+ "        ST_MakePoint(longitude, latitude, 4326)\r\n"
						+ "    )/1000 < (ST_DistanceSphere(ST_MakePoint("+longitude[0]+","+latitude[0]+",4326), ST_MakePoint("+longitude[1]+","+latitude[1]+",4326))/1000)\r\n"
						+ ") as S natural join placeLocation where categoryid="+categoryID+";");
			}
			else {
				ret = st.executeQuery("select category, placename, number, address, latitude, longitude,count, star\r\n"
						+ "from(\r\n"
	...
	public void updateStar(String city,String place, int star){
		...
		try {
			...
			ret = st.executeQuery("select count(*) from place where placename='"+place+"' and cityid="+cityID+";");
			while(ret.next()) {
				int tmp = Integer.parseInt(ret.getString(1));
				if(tmp==0) {
				...
				ret = st.executeQuery("select placeID from place where placename='"+place+"' and cityid="+cityID+";");
				ret.next();
			...
			ret=  st.executeQuery("select placename, count, star from place natural join (select customerid,placeid, count(star), round(avg(star)::numeric, 1) as star\r\n"
					+ "from star\r\n"
					+ "group by rollup(placeid,customerid)) as S"+
	...
	public void startService() {
		...
		while(true) {
			while(true) {
				System.out.format("1.테마별 관광지 조회\n2.도시별 관광지 조회\n3.테마 및 도시별 관광지 조회\n4.두 도시 이동 간의 추천 관광지\n5.후기 남기기\n0.종료\n");
				try {
				...
				case 4:
					...
					while(true) {
						try {
							System.out.format("\n조회 기준\n 1.카테고리 2.없음\n");
							System.out.format("정렬 기준 입력 : ");
							checkCategory = scan.nextInt();
							scan.nextLine();
							if(checkCategory!=1 && checkCategory!=2) {
								throw new InputMismatchException("올바른 값을 입력해주세요.");
							}
							break;
						}
					...
					if(checkCategory == 1) {
						...
						categoryID = scan.nextInt();
						scan.nextLine();
						readCoursePlace(srcCity, dstCity,true, categoryID);
					}
					else {
						readCoursePlace(srcCity, dstCity, false, 0);
					}
					break;
				case 5:
